...
def test_save_tokens_calls_db_save(mocker):
    ...
    mocker.patch.object(oauth_manager, "_encrypt_token", side_effect=lambda x: f"enc_{x}")
...
def test_save_tokens_db_error(mocker):
    ...
    mocker.patch.object(oauth_manager, "_encrypt_token", return_value="enc")
    mocker.patch.object(oauth_manager, "save_oauth_tokens", side_effect=Exception("db error"))
    expires = datetime(2020, 1, 1, tzinfo=timezone.utc)
    with pytest.raises(OAuthError) as exc:
...
def test_refresh_access_token_no_refresh(mocker):
    ...
    mocker.patch.object(oauth_manager, "get_tokens", return_value=("A", None, now, []))
    with pytest.raises(OAuthError) as exc:
...
def test_refresh_access_token_http_error(mocker):
    ...
    now = datetime.now(timezone.utc)
    mocker.patch.object(oauth_manager, "get_tokens", return_value=("A", "R", now, []))
...
def test_refresh_access_token_success(mocker):
    ...
    now = datetime.now(timezone.utc)
    scopes = ["x", "y"]
    mocker.patch.object(oauth_manager, "get_tokens", return_value=("A", "R", now, scopes))
    mocker.patch.object(oauth_manager.settings, "x_api_key", "key")
...
def test_get_valid_x_token_not_expired(mocker):
    ...
    mocker.patch.object(oauth_manager, "get_tokens", return_value=("OLD", None, future, []))
...
def test_get_valid_x_token_expired(mocker):
    ...
    mocker.patch.object(oauth_manager, "get_tokens", return_value=("OLD", None, soon, []))
    refresh_patch = mocker.patch.object(oauth_manager, "refresh_access_token", return_value="REF")

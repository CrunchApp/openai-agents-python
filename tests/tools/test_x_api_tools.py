...
from tools.x_api_tools import post_text_tweet, get_mentions, XApiError
...
def test_get_mentions_success_with_since(mocker):
    ...
    class FakeMentionsRes:
        status_code = 200
        def json(self):
            return {"data": [{"id": "m1"}], "includes": {"users": [{"id": "u1", "username": "user", "name": "User"}]}}
    calls = []
    ...
    assert result == {"data": [{"id": "m1"}], "includes": {"users": [{"id": "u1", "username": "user", "name": "User"}]}}
